/*
    ...
    Converts them to our custom test format.
*/
...
For key, test in jsonData.tests {
    ...
    tokens := "[T:Not implemented]"

    pos := 1
    fileName := ""
    while (pos := RegExMatch(title, "(\w+)", match, pos+StrLen(match))) {
        fileName .= match "-"
    }

    fileName := Format("{:L}", RTrim(fileName, "-") ".test")
    testData := Format(template, title, desc, data, t_, expected, tokens)

    outputDir := A_ScriptDir . "\test-data\<REPLACE_THIS>\"
    FileAppend, %testData%, %outputDir%\%fileName%
    msgBox, write
}
