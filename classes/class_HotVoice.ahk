Class HotVoice {
	...
	__New(){
		...
		dllName := "HotVoice.dll"
		dllFile := A_LineFile "\..\" dllName
		
		if (!FileExist(dllFile)){
			MsgBox % "Unable to find lib\" dllName ", exiting...`n"
			ExitApp
		}

		hintMessage := "Dll may be blocked. Try running the powershell command`nGet-ChildItem -Path '.' -Recurse | Unblock-File`nin the script folder"
