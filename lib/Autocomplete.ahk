...
_EnumString_QueryInterface(self,riid,pObj){
	DllCall("Ole32\StringFromCLSID","ptr",riid,"ptr*",sz),string:=StrGet(sz)
...
_EnumString_AddRef(self){
	return 1	; NumPut(nRefCount:=NumGet(self+A_PtrSize*8)+1,self+A_PtrSize*8)*0+nRefCount
}

_EnumString_Release(self){
	return 0	; NumPut(nRefCount:=NumGet(self+A_PtrSize*8)-1,self+A_PtrSize*8)*0+nRefCount
}

_EnumString_Skip(self,celt){
	;nCurrentElement:=NumGet(self+A_PtrSize*9)+celt
	;if nCurrentElement>sList.maxindex()
	;	nCurrentElement:=1
	return 0
}
