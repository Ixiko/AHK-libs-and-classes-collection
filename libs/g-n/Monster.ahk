MonsterEval(x,floatprecision:="0.14") {                              ; non-recursive PRE/POST PROCESSING: I/O forms, numbers, ops, ";"
   ...
   SetFormat FLOAT, 0.14e              ; Full intermediate float precision
   ...
   Else {
      if (RegExReplace(FormF,"[^.\d]")+0>RegExReplace(floatprecision,"[^.\d]"))
		tmpr:=FormF
	  else tmpr:=floatprecision
	  W := W="" ? tmpr : "0." . W    ; Set output form, Default = 6 decimal places
      SetFormat FLOAT, %W%
      y += 0.0
   }
