...
;~ portalStatus("http://www.google.com")
;~ portalStatus(url="http://portal.ntuh.gov.tw"){
	;~ try {
...
;~ msgbox % getUserName("100860","2B4124D6A69E491BB08F47AF9F1D367A")
...
return 0 if invalid
*/
;~ checksid("46CB9A66F388CB4568AF5D448DD97223F8")
checkSid(sid){
	global portalUrl
...
; parse lab data (old website) webpage html into object
/* structure:
name: patient name
...
;~ getLab4New("5772727","7B41E7A37F49482D9C79CCEC9C8F8C68")
;~ getLab4New(chartNo,sid,days=90){
	;~ url:="http://ihisaw.ntuh.gov.tw/WebApplication/ElectronicMedicalReportViewer/MedicalReportContent.aspx?SESSION=" sid "&PatClass=I&WardCode=09B&ChartNo=" chartNo "&HospitalCode=T0&Seed=" a_now "&IntervalDay=-" days
	;~ httprequest(url,html:="","","charset: utf-8")
	;~ clipboard:=html
;~ }

;~ findLab4New(getLab4New("3880641","BE164D69C80A490AB37529824EF16D13",20),"T-BIL")
...
;~ a:=patientBWBH("4262122","B631ABFB65014E259ED4C30B4D5EDCEE")
...
;~ getRisSchedule("F568CE2492064B999D3979B690657BD4","","CHCT","2013/03/04")
;~ html:=getRisPatientMainPage("F568CE2492064B999D3979B690657BD4",11)
;~ a:=parseRisPatientMainPage(html)
...
;~ getAccountIdFromAccNo("B728452E2D45454E8E527099157D5648","T0169538358")
...
;~ comobjerror(false)
;~ a:=getRisSchedule("5B30E0DF6ACE41C8B7EE837E989C9E93","5842539")
;~ b:=new parseRisSchedule(a)
;~ d:=b.query([["Date","2015/04/13"]])
;~ c:=1
...
;~ getPatho("97CDB02E597943BFAC35E57968A1354E","5508972")
...
;~ msgbox % portalMessage(sid,"0972653157","test")
...
;~ getOpHistory("5AC650F058384DE2BFB90BA005E3E433","3567897")
...
;~ clipboard:=queryDigiSign("D85480B694014C5781DE6D3E81E29FB8","100860")
...
;~ clipboard:=queryDigiSignN("7B0931CBC4F74860A8093D6C76AD8004","007382")
...
;~ a:=EMRViewerMobile("777C5B94A94B4A3C97F61C6C5345442E","8053310")
;~ b:=EMRgetRadExams(a)
;~ a.getReport("PA View","20130808")
...
;~ sid:="481338B4B66A45FF8F05A27C827BE3A1"
;~ chartNo:="5481590"
...
;~ PacsWebInfo(output,"T0134497741","8053310","100860","31416")
;~ a:=1
;~ PacsWebInfo(output,"T0159309773","3626012","100860","31416")
;~ PacsWebInfo(output,"T0158777424","6232248","100860","31416")
...
;~ httprequest("http://pacsmobile.ntuh.gov.tw/?user=100860&password=31416&tab=Display&PatientID=4384985&AccessionNumber=T0159353036&theme=epr",post,header)
;~ PacsWebDownloadJpeg2("http://pacsmobile.ntuh.gov.tw/wado/?v=2.0.SU3.3&requestType=XERO&studyUID=1.2.124.113532.80.22187.2757.20151230.105255.173046468&seriesUID=1.2.392.200046.100.14.787097720830906753119811747483526556630&objectUID=1.2.392.200046.100.14.545665725318203531646724020424983745249&language=zh_TW&ae=local","T0159353036","4384985","test.jpeg","","2500","100860","31416")

;~ ?requestType=WADO&studyUID=1.2.124.113532.80.22187.2757.20151230.105255.173046468&seriesUID=1.2.392.200046.100.14.787097720830906753119811747483526556630&objectUID=1.2.392.200046.100.14.545665725318203531646724020424983745249&ae=local

;~ output:=PacsWebInfoFlatten(PacsWebInfo(output,"T2134398558","N033777","100860","31416"))
...
;~ PacsWebDownloadJpeg2("http://pacsmobile.ntuh.gov.tw/wado/?v=2.0.SU3.3&requestType=XERO&studyUID=1.2.124.113532.80.22187.2757.20151230.105255.173046468&seriesUID=1.2.392.200046.100.14.787097720830906753119811747483526556630&objectUID=1.2.392.200046.100.14.545665725318203531646724020424983745249&language=zh_TW&ae=local","T0159353036","4384985","T0159353036 4384985 1.jpeg","C:\Documents and Settings\ntuhuser\My Documents\feedRIS\T0159353036 4384985",1531,"100860","31416")
...
;~ PacsWebDownloadJpeg("T0159353036","4384985","test.jpeg","","2500","100860","31416")
...
;hospital="1,2,3,4,5,6" -> 總院、金山、北護、新竹、竹東、雲林
; one page = 10 results
...
;; 院區 姓名 性別 年齡 生日 病歷號 Modality 檢查時間 AccessNo 檢查項目
...
*/
; need patient admission account no using getAccountIdFromAccNo(sid, AccNo)
;~ msgbox % a:=getAccountIdFromAccNo("DDD5E9C88C8648A3A3B000A608DA15C9", "T0169571394")
;~ clipboard:=getRisPatientHx(a:="16T02877448")
...
;~ a:=a_tickcount
;~ b:=getPacsHx("4B0B9AE6B3CD451FA1A4E972B0264483","8053310")
;~ msgbox % a_tickcount-a "," b.maxindex()
