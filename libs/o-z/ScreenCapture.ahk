;-------------------------------------------------------------------------
; The is Sean's ScreenCapture script:
; http://www.autohotkey.com/forum/topic18146.html
;
; It is modified to work with AutoHotKey_L.
; See the post from Sean on this page:
; http://www.autohotkey.com/forum/topic18146-120.html
;
; Here is the post:
; Making it 32-bit compatible requires only the following three changes
; in the script. 64bit needs more changes.
; Code:
;   InStr(Ansi4Unicode(...) --> InStr(StrGet(... , "UTF-16")
;   "Uint", Unicode4Ansi(wFileFr,sFileFr) --> "WStr", sFileFr
;   "Uint", Unicode4Ansi(wFileTo,sFileTo) --> "WStr", sFileTo
;-------------------------------------------------------------------------
...
Convert(sFileFr = "", sFileTo = "", nQuality = "") {
	...
	;Else	DllCall("gdiplus\GdipLoadImageFromFile", "Uint", Unicode4Ansi(wFileFr,sFileFr), "UintP", pImage); cb4960
  ...
  DllCall("gdiplus\GdipGetImageEncodersSize", "UintP", nCount, "UintP", nSize)
	...
	Loop, %	nCount
		;If	InStr(Ansi4Unicode(NumGet(ci,76*(A_Index-1)+44)), "." . sExtTo) ; cb4960
	...
	If	pImage
		;pCodec	? DllCall("gdiplus\GdipSaveImageToFile", "Uint", pImage, "Uint", Unicode4Ansi(wFileTo,sFileTo), "Uint", pCodec, "Uint", pParam) : DllCall("gdiplus\GdipCreateHBITMAPFromBitmap", "Uint", pImage, "UintP", hBitmap, "Uint", 0) . SetClipboardData(hBitmap), DllCall("gdiplus\GdipDisposeImage", "Uint", pImage) ; cb4960
		pCodec	? DllCall("gdiplus\GdipSaveImageToFile", "Uint", pImage, "WStr", sFileTo, "Uint", pCodec, "Uint", pParam) : DllCall("gdiplus\GdipCreateHBITMAPFromBitmap", "Uint", pImage, "UintP", hBitmap, "Uint", 0) . SetClipboardData(hBitmap), DllCall("gdiplus\GdipDisposeImage", "Uint", pImage) ; cb4960
