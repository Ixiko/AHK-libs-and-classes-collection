; Title:   	Gmail: click on first attachment - how? - AutoHotkey Community - Brave
; Link:   	autohotkey.com/boards/viewtopic.php?f=76&t=82212&hilit=acc+click
; Author:	malcev
; Date:   	19.10.2020
; for:     	AHK_L
...
setbatchlines -1
uia := UIA_Interface()
condition := uia.CreatePropertyCondition(UIA_ControlTypePropertyId := 30003, UIA_HyperlinkControlTypeId := 50005, VT_I4 := 3)

f11::
ControlGet, hChrome, hwnd,, Chrome_RenderWidgetHostHWND1, % "ahk_id " WinExist("A")
xd := uia.ElementFromHandle(hChrome)
for index, element in xd.FindAll(condition, TreeScope_Descendants := 0x4){
   if InStr(element.CurrentName, "Preview attachment")   {
