class OpenCV {
	...
	class Libs {
		...
		class __Init {
			...
			error(TheFunc := "", result := "", additionalMsg := "")	{
				Tooltip % additionalMsg
				stackTrace := (TheFunc "`nErrorlevel: " ErrorLevel)
				errorCode := "`nLastError: " A_LastError
				if ErrorLevel
					MsgBox % additionalMsg . "`n" . stackTrace . " " . errorCode
				return ErrorLevel
			}
		}
		
		__wrapAround(TheFunc, result := "") {
			this.error(TheFunc, result)
			return result
		}
		...
		Load(cvfilename, cvmemstorage := 0, cvname := 0, cvreal_name := 0) {
			...
			return this.__wrapAround(A_ThisFunc, result)
		}
		...
		ArrTohBitmap(pimage, byref width = "", byref height = "") { 
			...
			Return this.__wrapAround(A_ThisFunc, hBitmap), this.ReleaseImage(result)
		}
		
		Scalar(ByRef vScalar, value)
		{
		...
		GetSize(pimage, ByRef width := 0, ByRef height := 0) {
			...
			return this.__wrapAround(A_ThisFunc, WidthHeight)
		}
		
		Size(width, height) {
			return this.__wrapAround(A_ThisFunc, width & 0xFFFFFFFF | height << 32)
		}
		...
		Point(x, y) {
			return this.__wrapAround(A_ThisFunc, x & 0xFFFFFFFF | y << 32)
		}
		
		RGB(cvRed, cvGreen, cvBlue, ByRef storage := "") {
			VarSetCapacity(storage, 32, 0)
			NumPut(cvBlue, storage, 0, "double")
			NumPut(cvGreen, storage, 8, "double")
			NumPut(cvRed, storage, 16, "double")
			NumPut(0, storage, 24, "double")
			return this.__wrapAround(A_ThisFunc, storage)
		}
		...
		{
			result := this.opencv_imgproc("cvSmooth", "ptr", cvsrc, "ptr", cvdst, "int", cvsmoothtype, "int", cvsize1, "int", cvsize2, "double", cvsigma1, "double", cvsigma2, "Cdecl")
			return this.__wrapAround(A_ThisFunc, result)
		}
		...
		;
		;
		FindContours( 	cvimage, cvstorage, cvfirst_contour, cvheader_size, cvmode, cvmethod, cvoffset)
		{
			result := this.opencv_imgproc("cvFindContours" , "ptr" , cvimage , "ptr" , cvstorage , "Ptr*" , cvfirst_contour , "int" , cvheader_size , "int" , cvmode , "int" , cvmethod , "int64" , cvoffset, "Cdecl")
			return this.__wrapAround(A_ThisFunc, result)
		}
