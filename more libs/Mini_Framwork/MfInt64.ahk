...
*/
	GetValue(args*) {
		...
		else if (i = 2)
		{
			try
			{
				_default := MfInt64._GetValue(args[2])
			}
			catch e
			{
				CanThrow := true
	...
	_GetValue(obj) {
		...
		try
		{
			...
			if (IsObject(obj)) {
				...
				{
					if ((obj.LessThen(MfInt64.MinValue)) || (obj.GreaterThen(MfInt64.MaxValue))) {
						ex := new MfArgumentOutOfRangeException("obj"
							, MfEnvironment.Instance.GetResourceString("ArgumentOutOfRange_Bounds_Lower_Upper"
							,MfInt64.MinValue, MfInt64.MaxValue))
						ex.SetProp(A_LineFile, A_LineNumber, A_ThisFunc)
				...
				retval := MfInt64._GetValueFromVar(obj)
			}
		}
		Catch e
		{
	...
	_GetValueFromVar(varInt) {
		...
		try
		{
			...
			if (Mfunc.IsInteger(varInt)) {
				...
				if ((sf.LessThen(MfInt64.MinValue)) || (sf.GreaterThen(MfInt64.MaxValue))) {
					ex := new MfArgumentOutOfRangeException("varInt"
						, MfEnvironment.Instance.GetResourceString("ArgumentOutOfRange_Bounds_Lower_Upper"
						,MfInt64.MinValue, MfInt64.MaxValue))
					ex.SetProp(A_LineFile, A_LineNumber, A_ThisFunc)
		...
		try {
			...
			if (strP = "MfChar") {
				...
				if (RegExMatch(strV, "^\s*([-+]?\d{1,19})\s*$", match)) {
					iVal := MfInt64.GetValue(match1)
					if ((iVal >= MfInt64.MinValue) && (iVal <= MfInt64.MaxValue)) {
						retval := iVal
					}
	...
	TryParse(byref int, args*) {
		...
		if (IsObject(Int)) {
			if (MfObject.IsObjInstance(int, "MfInt64")) {
				...
				; Int is an object but not an MfInt64 instance
				; only MfInt64 is allowed as object
				return false
			}
		...
		try {
			...
			if (strP == "MfChar")
			{
				...
				if (MfChar.IsDigit(c)) {
					if (_isObj = true) {
						int.Value := MfInt64.GetValue(MfCharUnicodeInfo.GetDecimalDigitValue(c))
					} else {
						int := MfInt64.GetValue(MfCharUnicodeInfo.GetDecimalDigitValue(c))
					}
					retval := true
				}
			}
