...
*/
	__New(int:=0, retunAsObj:=false, readonly:=false) {
		...
		_intResult := MfUInt16.GetValue(int,"NaN", true)
		if (_intResult == "NaN")
		{
			ex := new MfArgumentException(MfEnvironment.Instance.GetResourceString("InvalidCastException_ValueToUInt16"))
			ex.SetProp(A_LineFile, A_LineNumber, A_ThisFunc)
			throw ex
		}
		_int := _intResult
	...
	Parse(args*) {
		...
		try {
			...
			if (strP = "MfString" || strP = "MfChar")
			{
				...
				ns := 7 ; integer
				retval := MfUInt16._Parse(strV, ns,  MfNumberFormatInfo.CurrentInfo, A_ThisFunc)
			}
			else if (cnt = 2)
			{
				...
				{
					ns := 7 ; integer
					retval := MfUInt16._Parse(str.Value, ns, MfNumberFormatInfo.GetInstance(obj), A_ThisFunc)
				}
				else if (MfObject.IsObjInstance(obj, MfNumberStyles))
				{
					retval := MfUInt16._Parse(str.Value, obj.Value, MfNumberFormatInfo.CurrentInfo, A_ThisFunc)
				}
				else
				{
					ex := new MfNotSupportedException(MfEnvironment.Instance.GetResourceString("NotSupportedException_MethodOverload", A_ThisFunc))
			...
			else if (cnt = 3)
			{
				...
				retval := MfUInt16._Parse(str.Value, ns.Value, MfNumberFormatInfo.GetInstance(fInfo), A_ThisFunc)
			}
...
*/
	TryParse(byref int, args*) {
		...
		if (strP = "MfString" || strP = "MfChar")
		{
			strV := objParams.Item[0].Value
			ns := 7 ; integer
			retval := MfUInt16._TryParse(strV, ns, MfNumberFormatInfo.CurrentInfo, num)
		}
		else if (cnt = 2)
		{
			str := objParams.Item[0]
			...
			{
				retval := MfUInt16._TryParse(str.Value, obj.Value, MfNumberFormatInfo.CurrentInfo, num)
			}
			else
			{
				ex := new MfNotSupportedException(MfEnvironment.Instance.GetResourceString("NotSupportedException_MethodOverload", A_ThisFunc))
		...
		else if (cnt = 3)
		{
			...
			retval := MfUInt16._TryParse(str.Value, ns.Value, MfNumberFormatInfo.GetInstance(fInfo), num)
		}
		else if (strP = "MfUInt16")
		{
