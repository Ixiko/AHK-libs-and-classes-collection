/*
    For the Tab and Tab2 controls, the position _and_size of the GUI controls in
    a tab may need to be changed when the tab control is resized.  This example
    shows one way of doing it using the TAB_GetDisplayArea function.

    Note: This is a relatively simple example.  Repositioning a large number of
    controls can be complex and code intensive.  Because AutoHotkey does not
    automatically make changes to the position of the controls when the size of
    tab control changes, functions like Anchor, Attach, and AutoXYWH can be used
    instead.
*/
#NoEnv
...
;-- Tab control
gui Add,Tab2,w%TCWidth% h%TCHeight% hWndhTab vMyTab %TabStyle%,%Vegetables%
...
;-- Tab 1 and 2
GUIControl MoveDraw,Tab1_Text,x%ControlX% y%ControlY% w%ControlW% h%ControlH%
GUIControl Move,    Tab2_Edit,x%ControlX% y%ControlY% w%ControlW% h%ControlH%
...
GUIControl
    ...
    ,Tab4_Edit2
    ,% ""
        . "x" . ControlX+Round(ControlW/3) . A_Space
        . "y" . ControlY . A_Space
        . "w" . Round(ControlW/3) . A_Space
        . "h" . ControlH . A_Space
...
GUIControl
    ...
    ,Tab4_Edit3
    ,% ""
        . "x" . ControlX+(Round(ControlW/3)*2) . A_Space
        . "y" . ControlY . A_Space
        . "w" . Round(ControlW/3) . A_Space
        . "h" . ControlH . A_Space
...
GUIControl
    ...
    ,% ""
        . "x" . ControlX . A_Space
        . "y" . ControlY+Round(ControlH/3) . A_Space
        . "w" . ControlW . A_Space
        . "h" . Round(ControlH/3) . A_Space
...
GUIControl
    ...
    ,% ""
        . "x" . ControlX . A_Space
        . "y" . ControlY+(Round(ControlH/3)*2) . A_Space
        . "w" . ControlW . A_Space
        . "h" . Round(ControlH/3) . A_Space
...
GUIControl
    ...
    ,Tab3_MonthCal
    ,% ""
        . "x" . ControlX+Round(ControlW/2) . A_Space
        . "y" . ControlY . A_Space
        . "w" . Round(ControlW/2) . A_Space
        . "h" . ControlH . A_Space
return
