/*
    In this example, the color of the tabs are set.

    Disclaimer: This is a very limited example that was created by trial and
...
;-- Add the tab control
TabStyle :=0
gui Add,Tab3,xm w300 h250 %TabStyle% hWndhTab gSelectedTab vMyTab,A-C|D-F|G-J|K-M|N-Q|R-T|U-Z
gui Font

;-- Increase the height of the tabs so that the tab colors are a bit more
;   prominent.
...
;-- After this, all GUI controls are added outside of the tab control
gui Tab
...
OnDrawItem(wParam,lParam,Msg,hWnd)
    ...
    */
    ...
    if FirstCall
        ...
        ;-- Create new fonts using the attributes of the tab control's font.
        ;   This is not the same as cloning.  Not sure why.
        ;   Note: In this example, the fonts remain static and are not deleted
        ;   until the script ends.  This may not be the best approach in all
        ;   situations.
        hFontSelected  :=Fnt_CreateFont(Fnt_GetFontName(hFont),Fnt_GetFontOptions(hFont) . " Bold")
        hFontUnselected:=Fnt_CreateFont(Fnt_GetFontName(hFont),Fnt_GetFontOptions(hFont))
        ...
        ;------------------------
        Count:=0
        MaxColors:=TAB_GetItemCount(hTab)
        Loop
            {
            Random,Color,0x000000,0xFFFFFF
                ;-- Color is used as BGR so this color should be assumed to be
                ;   in BGR format.
            ...
            G:=((Color>>8)&0xFF)/255
            B:=((Color>>16)&0xFF)/255
            HSV:=HSV_Convert2HSV(R,G,B)

            ;-- Pastel(ish) colors only
            ;   Note: Pastel colors are used because they are easy to view but
            ;   also because it doesn't require the text color to change.  This
            ;   will work on most themes because the text color is black but it
            ;   won't always work as expected.  On themes with dark background
            ;   and light text color, the results may not be usable.
            if (HSV.V<0.7 or HSV.S>0.4)
                Continue
    ...
    if (itemState=ODS_SELECTED)
        {
        hBrush:=DllCall("CreateSolidBrush","UInt",Color%iTab%)
        DllCall("FillRect","Ptr",hDC,"Ptr",lParam+(A_PtrSize=8 ? 40:28),"Ptr",hBrush)
        DllCall("DeleteObject","Ptr",hBrush)

        ;-- Set the text background color
        DllCall("SetBkColor","Ptr",hDC,"Int",Color%iTab%)
    ...
    ;   not draw the tab when is this done.
    Return True
    }
