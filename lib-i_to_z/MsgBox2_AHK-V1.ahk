...
Global MsgBox2InputHook, MsgBox2Result
MsgBox2(sMsg,title:="Notification",options:="") {
	...
	SysGet, tempMaxW, 78
	maxW := (maxW>tempMaxW?tempMaxW-150:maxW) ; final msg width estimate, mimic MsgBox minimum width
	If (msg.h>maxH)
		vScroll := "", forceSelectable := true
	Else
		vScroll := "-VScroll"

	Gui, MsgBox2:New, -SysMenu +HwndMsgBox2hwnd +LabelMsgBox2, %title%
	If (bgColor)
		Gui, Color, %bgColor%
	...
	If (iconFile) {
		If (iconHandle)
			Gui, MsgBox2:Add, Picture, w32 h-1, %iconFile%
		Else If (iconNum)
			Gui, MsgBox2:Add, Picture, Icon%iconNum% w32 h-1, %iconFile%
		Else
			Gui, MsgBox2:Add, Picture, w32 h-1, %iconFile%

		GuiControlGet, picCtl, Pos, Static1 ; ClassNN name for Pic/Icon if it is created before txt ctl

		icon := {}, icon.h := picCtlH, icon.w := picCtlW, icon.x := picCtlX, icon.y := picCtlY
		spacerH := icon.x, newXY := "x" (spacerH*2)+32 " yp" ; define some dimensions
	} Else
		spacerH := msg.x, newXY := "" ; define some dimensions
	...
	If (!selectable) {
		Gui, MsgBox2:Add, Text, %newXY% w%maxW% h%maxH%  %txtColor%, %sMsg%
		GuiControlGet, tempMsg, Pos, %ctl%
		msg := {}, msg.x := tempMsgX, msg.y := tempMsgY, msg.h := tempMsgH, msg.w := tempMsgW
	} Else {
		SysGet, vScrollW, 2
		newW := maxW + vScrollW + 5
		r := maxH / txtH ; compensate for no VScroll / get rows of text

		Gui, MsgBox2:Add, Edit, %newXY% w%newW% r%r% %vScroll% ReadOnly -Tabstop -E0x200 +Background%bgColor% %txtColor%
		GuiControlGet, tempMsg, Pos, Edit1
		msg := {}, msg.x := tempMsgX, msg.y := tempMsgY, msg.h := tempMsgH, msg.w := tempMsgW
		GuiControl,, Edit1, %sMsg%
	}
	...
	If (IsObject(icon)) {
		If (msg.h < icon.h) { ; adjust text if icon and certain txt dimensions
			...
			GuiControl, MsgBox2:Move, Static1, y%newY%
	...
	Loop Parse, btnList, | ; list specified buttons
	{
		If (A_LoopField) { ; make sure button exists, helpful on zero-length string
			...
			If (A_Index = 1)
				Gui, MsgBox2:Add, Button, gMsgBox2event x%nX% y%nY% w%bW% h%bH% +BackgroundTrans %def%, %btnText%
			Else
				Gui, MsgBox2:Add, Button, gMsgBox2event x+0 w%bW% h%bH% %btnProp% %def%, %btnText%
		}
	}
	If (help) ; add help button if specified
		Gui, MsgBox2:Add, Button, gMsgBox2event x+0 w%helpW% h%helpH%, %help%

	If (cHwnd) { ; modal/owner has already been determined/consolidated to "cHwnd"
		Gui, +Owner%cHwnd%
		If (modal)
			WinSet, Disable,, ahk_id %cHwnd%
	}
	...
	If (dX And dY) {
		...
		Gui, MsgBox2:Show, w%w% h%h% x%dX% y%dY%
	} Else
		Gui, MsgBox2:Show, w%w% h%h%
...
MsgBox2event(CtrlHwnd, GuiEvent, EventInfo) {
	...
	MsgBox2Result := btnText ":" btnClassNN
...
GetMsgDimensions(sString,sFaceName,nHeight,maxW := 0) {
	...
	lineH := curCtlH
	Gui, Destroy ; ====================== end get line height

	maxH := 0 ; maxW is already set
	fullLen := StrLen(sString)
	curPos := 1, len := 65535

	While (curPos <= fullLen) { ; loop through text 65,535 chars at a time
		tinyStr := SubStr(sString,curPos,len)

		Gui, New
		Gui, Font, s%nHeight%, %sFaceName%
		Gui, Add, Text,, %tinyStr%

		GuiControlGet, ctlSizeTmp, Pos, Static1
		ctlSize := {}
		ctlSize.h := ctlSizeTmpH, ctlSize.w := ctlSizeTmpW
		ctlSize.x := ctlSizeTmpX, ctlSize.y := ctlSizeTmpY
