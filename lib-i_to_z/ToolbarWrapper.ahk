...
{
	Global
	%szToolbarName%_hToolbar					:= 0		; 工具栏句柄
	%szToolbarName%_hProcess					:= 0		; 工具栏所属进程句柄
	%szToolbarName%_nProcessId				:= -1		; 工具栏所属进程ID
	%szToolbarName%_pLastAllocatedAddr	:= -1		; 上一次申请空间时分配的地址
	%szToolbarName%_nLastAllocatedSize		:= -1		; 上一次申请空间时分配的大小
}
...
; 从toolbar所属进程中获得指定大小的内存，如果已有足够大小的内存曾被分配过则不重新申请，否则重新申请
