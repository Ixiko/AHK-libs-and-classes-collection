         ...
         if(MatrixStatic.RowCount(m) != 1)
         {
            for each, row in m
            {
               for each, item in row   
                  prnt .= item colDel
               if each
                  prnt:=substr(prnt,1,(-1)*strlen(colDel))
			   prnt .= rowDel
            }
            if each
                  prnt:=substr(prnt,1,(-1)*strlen(rowDel))
...
class Matrix extends MatrixStatic
{
   ...
   * Returns a console/msgbox friendly string. Useful for debugging
   */
   ToString(m=0, colDel=" ", rowDel="`n"){
      if (this != Matrix)
         m := this
      return base.ToString(m, colDel, rowDel)    
   }
